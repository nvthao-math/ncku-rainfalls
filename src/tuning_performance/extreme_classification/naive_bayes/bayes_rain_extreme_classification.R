...
rain.extreme <- data.season.rain[ind.extreme.rain, ]
# normal rain data
ind.normal.rain <- which(data.season.rain$clazz == 1) 
rain.normal <- data.season.rain[ind.normal.rain, ]
#
...
# # file to save
file <- "/home/bigdata/workspace/project/r/ncku-rainfalls/result/bayes_rain_output_extreme_normal.txt"
#
threshold <- 0.6
for(i in c(7338:(2^n.feature-1))){
  ...
  folds <- cut(seq(1,nrow(z.data)),breaks=10,labels=FALSE)
  ...
  for(j in 1:10){
    ...
    model <- naiveBayes(data.xtrain, ytrain)
    ytest <- factor(data.ytest, labels = c("yes", "no"))
    data.ypred.test <- predict(model,data.xtest)
    ...
    data.ypred.train <- predict(model, data.xtrain)
  ...
  if(extreme.acc.test > threshold){
    ...
    write.csv(train2BuildModel, file = "/home/bigdata/workspace/project/r/ncku-rainfalls/data/buildModel/extrem_vs_normal/lda_rain_trainData.csv")
  }
}
