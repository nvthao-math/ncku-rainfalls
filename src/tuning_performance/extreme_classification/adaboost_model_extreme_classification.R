...
# # file to save
bayes.file <- "/home/bigdata/workspace/project/r/ncku-rainfalls/result/adaboost_output_extreme_normal.txt"
#
for(i in c(33553408:(2^n.feature-1))){
  ...
  data.x <- data.frame(cbind(data.xtrain, bayse.ytrain))
  
  iris.rf <- adaboost(bayse.ytrain ~ ., data.x, 10)
  bayes.data.ypred <- (predict(iris.rf, data.xtest))$class
  # accuracy
  extreme.acc <- c(sum(bayes.data.ypred[which(data.ytest==2)] == 2)/(sum(data.ytest==2)))
  normal.acc <- c(sum(bayes.data.ypred[which(data.ytest==1)]==1)/(sum(data.ytest==1)))
  bayes.acc <- c(sum(bayes.data.ypred==data.ytest)/length(data.ytest))
